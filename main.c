...
int main()
{
    ...
    int INPUT_VECTOR_SIZE = 5;
    int HIDDEN_VECTOR_SIZE = 4;
    ...
    gsl_vector * true_out_val = gsl_vector_alloc(OUTPUT_VECTOR_SIZE);

    // init weights
    randomizeMatrix(w1, INPUT_VECTOR_SIZE, HIDDEN_VECTOR_SIZE);
    randomizeMatrix(w2, HIDDEN_VECTOR_SIZE, OUTPUT_VECTOR_SIZE);

    //printf("\n%f\n", gsl_matrix_get (w1, INPUT_VECTOR_SIZE-1, HIDDEN_VECTOR_SIZE-1));
    //printf("\n%f\n", gsl_matrix_get (w2, HIDDEN_VECTOR_SIZE-1, OUTPUT_VECTOR_SIZE-1));
...
int index = 0;
    ...
    while(1)
    {
        ...
        j++;
        // TODO count vectory by this index later and get rid of i and j...
        index++;
        ...
        if (i == 28)
        {
            ...
            gsl_vector_get(output, 3),
            gsl_vector_get(output, 4),
            gsl_vector_get(output, 5),
            gsl_vector_get(output, 6),
            gsl_vector_get(output, 7),
            gsl_vector_get(output, 8),
            gsl_vector_get(output, 9));
            matrixIdx++;

            // backpropagate
            //desired - actual
            printf("Value: %d\n", c);
            // TODO should evaluate output agains actual value here
            gsl_vector_set_basis (actual_out, c);
            gsl_vector_sub(true_out_val, output);
            gsl_vector_scale(true_out_val, sigmoid_derivative());
            break;
        }
